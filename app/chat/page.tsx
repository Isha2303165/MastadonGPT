...
ChatInterface.propTypes = {
  ...
  redirectOnMessage: PropTypes.bool,
};
...
interface ChatInterfaceProps {
  onClose?: () => void;
  redirectOnMessage?: boolean;
}

export default function ChatInterface({ onClose, redirectOnMessage = false }: ChatInterfaceProps) {
  const [messages, setMessages] = useState<{ sender: string; text: string; isError?: boolean }[]>([]);
  ...
  useEffect(() => {
    ...
    console.log('Initial message from query:', initialMessage); // Debugging log
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // If redirectOnMessage is true and user has not been redirected
    if (redirectOnMessage && !hasRedirected) {
      setHasRedirected(true);

      const queryString = new URLSearchParams({ message: userMessage.text }).toString();
      console.log(`/chat?${queryString}`); // Debugging log
    ...
    try {
      ...
      const chatHistory = data.map((item: { sender: string; message: string; id?: string }) => ({
      ...
      const botResponseWithId = chatHistory.find((msg: { sender: string; text: string; id?: string }) => msg.sender === 'bot' && msg.id);
  ...
  return (
    <div className="flex flex-col h-screen w-screen p-4 bg-gray-100"> {/* Outer container */}
      <div className="flex flex-col h-full w-full max-w-4xl mx-auto border border-gray-300 rounded-lg shadow-lg bg-white overflow-hidden"> {/* Added border */}
        {onClose && (
          <button
            onClick={onClose}
